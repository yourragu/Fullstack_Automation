...
public class TestNG_CustomerServiceOption {
	...
	public void nonProfit() throws InterruptedException, AWTException, IOException {
		...
		action.moveToElement(driver.findElement(By.xpath("//li[@id='products_menu_item']")))
				.moveToElement(driver.findElement(By.xpath("//*[contains(@class,'cloud-icon-service')]"))).click()
				.build().perform();
		List<WebElement> ele = driver.findElements(By.xpath("//ul[contains(@class,'searchEnabled')]/li"));
		for (int i = 1; i < ele.size(); i++) {
			System.out.println(
					driver.findElement(By.xpath("//ul[contains(@class,'searchEnabled')]/li[" + i + "]/button/span[1]"))
							.getText());
		}
